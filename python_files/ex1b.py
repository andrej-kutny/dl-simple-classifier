...
def run(source_dir, results_dir, epochs, learning_rate, batch_size, validation_split, seed):
    ...
    # Confusion matrix on validation set (shown as heatmap, labels omitted for readability at 120 classes)
    all_labels = []
    all_preds = []
    ...
    for images, labels in val_ds:
        ...
        pred_classes = np.argmax(np.array(probs), axis=1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train a Stanford Dogs classifier")
    parser.add_argument("-s", "--source-dir", default="data/stanford_dogs_images",
                        help="Path to the image dataset directory (default: data/stanford_dogs_images)")
